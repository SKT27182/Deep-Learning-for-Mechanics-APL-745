...
for i in range(5):
  ax[0, i].imshow(test_images[i], cmap='gray')
  ax[0, i].set_title('Predicted: %d' % np.argmax(forward(test_images[i], test_labels[i])[0]))
  ax[1, i].imshow(test_images[i+5], cmap='gray')
  ax[1, i].set_title('Predicted: %d' % np.argmax(forward(test_images[i+5], test_labels[i+5])[0]))