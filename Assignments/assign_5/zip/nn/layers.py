...
class Layer:
    ...
    def __init__(self):
        ...
        self.optimizer_w = Adam()
        self.optimizer_b = Adam()
...
class DenseLayer(Layer):
    ...
    def __init__(self, input_shape=None,units=None, activation=None, l1=0, l2=0, use_bias=True):
    ...
    def initialize(self, optimizer=Adam(), initializer="glorot_normal"):
        """
...
class ConvLayer(Layer):
    ...
    def __init__(self,  input_shape=None, filters=5, kernel_size=(3,3), strides=(1,1), padding="valid", use_bias=True):
        super().__init__()
    ...
    def _get_shapes(self):
        ...
        output_height = (self.input_height + pad_top + pad_bottom - kernel_height) // stride_height + 1
        output_width = (self.input_width + pad_left + pad_right - kernel_width) // stride_width + 1

        self.output_shape = (output_height, output_width, self.filters)


    def initialize(self, optimizer=Adam(), initializer="glorot"):
        self.optimizer_w = deepcopy(optimizer)

        self.weights = initializers(method=initializer, shape=(self.kernel_size[0], self.kernel_size[1], self.input_shape[2], self.filters)) # shape (kernel_height, kernel_width, prev_layer_filters, filters)
            ...
            for height_start in range(0, out_h, stride_height):
                for width_start in range(0, out_w, stride_width):
                    output[height_start, width_start, n_filter, :] = np.sum(
                        images[height_start:height_start + kernel_height , width_start:width_start + kernel_width, :, :] * kernel[:, :, :, n_filter, np.newaxis],
                        axis=(0, 1, 2)
    ...
    def forward_propagation(self, inputs):
        ...
        # Pad the input
        inputs = np.pad(inputs, ((self.pad_top, self.pad_bottom), (self.pad_left, self.pad_right), (0, 0), (0, 0)), mode="constant")
    ...
    def backward_propagation(self, dL_doutput):
        ...
        for n_filter in range(self.filters):
            for height_start in range(0, dL_doutput.shape[0], self.strides[0]):
                for width_start in range(0, dL_doutput.shape[1], self.strides[1]):
                    # flip the weights
                    fliped_kernel = np.flip(self.weights[:, :, :, n_filter], axis=(0, 1))
                    dL_dinput[height_start:height_start + self.kernel_size[0], width_start:width_start + self.kernel_size[1], :, :] += fliped_kernel[:, :, :, np.newaxis] * dL_doutput[height_start, width_start, n_filter, :]
                    ...
                    dL_dkernel[:, :, :, n_filter] += np.sum(input[height_start:height_start + self.kernel_size[0], width_start:width_start + self.kernel_size[1], :, :] * dL_doutput[height_start, width_start, n_filter, :], axis=3)
                    dL_dbias[n_filter] += np.sum(dL_doutput[height_start, width_start, n_filter, :])
        ...
        dL_dinput = dL_dinput[self.pad_top:dL_dinput.shape[0] - self.pad_bottom, self.pad_left:dL_dinput.shape[1] - self.pad_right, :, :]
                ...
                for width_start in range(0, dL_doutput.shape[1], self.strides[1]):
                    for channel in range(input.shape[2]):
                        ...
                        dL_dinput[
                            ...
                            channel,
                            image_,
                        ] += dL_doutput[height_start, width_start, channel, image_] * mask
