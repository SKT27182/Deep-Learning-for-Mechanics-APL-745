...
class NeuralNetwork:
    ...
    # set loss to use
    def compile(self, loss, optimizer=Adam(), initializer="glorot_uniform", metrics=None
     ):
    ...
    def display_metrics(self, y_pred, y_true):
        metrics_record = ""
        data = ""
        ...
        if data == "binary":
            ...
            if "recall" in self.metrics:
                metrics_record += f"recall: {metrics.recall(y_true, y_pred,  average='binary'):.4f} "
        ...
        if data == "multi-class":
            ...
            if "recall" in self.metrics:
                metrics_record += f"recall: {metrics.recall(y_true, y_pred,  average='macro'):.4f} "
            if "f1_scrore" in self.metrics:
        ...
        if verbo !=0:
            ...
            if verbo ==3:
                if (batch_iter) % 50 == 0:
                    ...
                    print(f"Epoch {iter+1}-{self.epochs} Batch {batch_iter+1}-{self.batches}", end=" ======================> ")
                    predicted = self.predict(x_train)

                    print(f"cost: {self.loss(y_train.T, predicted.T, ws):.4f}", end=" ")
                    if self.metrics is not None:
                        print(self.display_metrics(predicted, y_train))
